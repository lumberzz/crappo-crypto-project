(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && (e.target.closest(".icon-menu") || e.target.closest(".menu__body"))) if (!(bodyLockStatus && e.target.closest(".menu__list"))) {
                bodyLockToggle();
    ...
    function roundDecimal(number) {
        let roundedNumber;
        if (number < 10) roundedNumber = +number.toFixed(8); else if (number < 100) roundedNumber = +number.toFixed(6); else if (number < 1e3) roundedNumber = +number.toFixed(4); else roundedNumber = +number.toFixed(2);
    ...
    async function calculateRevenue() {
        ...
        const khRate = 1e3 * hashrate.value;
        const rate = khRate / Math.pow(1e3, parseInt(script_select.value));
        const revenue = rate * ethereumPrice * 24;
        output.innerHTML = roundDecimal(revenue) + " ETH";
        outputUSD.innerHTML = "($" + roundDecimal(revenue * ethereumPrice) + ")";
    }
    window["FLS"] = true;
    isWebp();
    menuInit();
